...
(function () {
  ...
  function formHtml(aq) {
    ...
    return `<section class="panel aq-3d-builder">
      ...
      <div class="map-actions"><button class="primary" onclick="saveAquarium3DSize()">${standalone ? 'Aplicar medidas al diseño' : 'Guardar medidas de la urna'}</button><button onclick="resetAquarium3DView()">Centrar vista</button></div>
  ...
  function applyDraft(d) {
    ...
    aq.display_net_liters = Number(l.water.toFixed(2));
    aq.system_net_liters = Number(l.water.toFixed(2));
    aq.real_liters = Number(l.water.toFixed(2));
    aq.liters = Number(l.water.toFixed(2));
    aq.volume_liters = Number(l.water.toFixed(2));
    return aq;
  }
  ...
  async function save() {
    ...
    const d = readForm();
    const l = liters(d);
    if (Number(byId('aq3dWater')?.value) > d.height) {
    ...
    const payload = {
      ...
      display_net_liters: Number(l.water.toFixed(2)),
      system_net_liters: Number(l.water.toFixed(2)),
      real_liters: Number(l.water.toFixed(2)),
      liters: Number(l.water.toFixed(2)),
      volume_liters: Number(l.water.toFixed(2))
    };

    if (aq.__standalone_3d) {
      Object.assign(aq, payload);
      if (box) box.innerHTML = msg(`Diseño actualizado: ${d.length} × ${d.width} × ${d.height} cm.`, 'success');
      if (R().renderMap3D) R().renderMap3D(window.__aqMap);
      return;
    }
  ...
  window.ANX.MapBuilder = { dimensions, liters, formHtml, readForm, preview, save, resetView };
})();