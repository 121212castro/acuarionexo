...
(function () {
  ...
  function money(value, currency) {
    try { return new Intl.NumberFormat('es-ES', { style: 'currency', currency: currency || 'EUR', minimumFractionDigits: 2, maximumFractionDigits: 4 }).format(Number(value) || 0); }
    catch (_) { return `${Number(value || 0).toFixed(4)} ${currency || ''}`; }
  }
  ...
  function dailyRows(items) {
    ...
    return `<div class="admin-analytics-daily">${items.map(item => {
  ...
  function analyticsStyles() {
    ...
    style.textContent = `.admin-analytics-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}.admin-analytics-list{display:grid;gap:8px}.admin-analytics-row{display:flex;justify-content:space-between;gap:12px;padding:10px 0;border-bottom:1px solid rgba(128,128,128,.18)}.admin-analytics-row:last-child{border-bottom:0}.admin-analytics-daily{display:grid;gap:10px}.admin-analytics-day{display:grid;grid-template-columns:90px minmax(120px,1fr) 50px 90px;align-items:center;gap:10px}.admin-analytics-bar-wrap{height:10px;border-radius:999px;background:rgba(128,128,128,.16);overflow:hidden}.admin-analytics-bar{height:100%;border-radius:999px;background:currentColor;opacity:.72}.admin-external-summary{border:1px solid rgba(70,180,255,.45);box-shadow:0 0 0 1px rgba(70,180,255,.08) inset}.admin-external-summary .quick-actions{margin-top:10px}.admin-analytics-secondary{opacity:.9}.ai-customer-grid{display:grid;gap:12px}.ai-customer-row{border:1px solid rgba(128,128,128,.2);border-radius:12px;padding:13px}.ai-customer-row .line{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap}.ai-status{font-weight:700}.ai-billing-form{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:12px}.ai-billing-form .wide{grid-column:1/-1}@media(max-width:640px){.admin-analytics-day{grid-template-columns:78px 1fr 36px}.admin-analytics-day small{grid-column:2/4}}`;
    document.head.appendChild(style);
  }

  window.adminAnalytics = async function () {
    const requireAdmin = ANX.Admin?.requireAdmin;
    ...
    try {
      ...
      const d = data || {};
      ANX.render(`
        <section class="summary-card"><div><small>Admin</small><h2>Analítica</h2><p>Personas externas separadas de las vistas de trabajo</p></div></section>

        <section class="panel admin-external-summary">
          <div class="panel-head"><h2>Personas externas</h2><small>Últimos 30 días</small></div>
          <div class="quick-actions">
            ${metric('Visitantes externos', d.accessVisitors30d || 0, 'Vieron la pantalla de acceso')}
            ${metric('Intentaron entrar', d.accessFormOpens30d || 0, 'Abrieron la solicitud de acceso')}
            ${metric('Solicitaron acceso', d.accessRequests30d || 0, 'Solicitudes reales recibidas')}
            ${metric('Pendientes', d.accessPending || 0, 'Esperan tu aprobación ahora')}
          </div>
          <p class="small"><b>Estos son los números importantes para saber cuánta gente está intentando entrar.</b> Las visitas y aperturas solo incluyen sesiones con analítica aceptada y excluyen navegadores reconocidos como administrador. Las solicitudes y pendientes proceden directamente del registro de permisos.</p>
        </section>

        <section class="panel admin-analytics-secondary">
          <div class="panel-head"><h2>Actividad de la aplicación</h2><button onclick="adminPanel()">← Admin</button></div>
          <p class="small">Los números siguientes son vistas y sesiones, no personas. Pueden incluir mucha actividad repetida de trabajo dentro de la app.</p>
          <div class="quick-actions">
            ${metric('Activos ahora', d.activeNow || 0, 'Sesiones con actividad en los últimos 5 min')}
            ${metric('Vistas hoy', d.visitsToday || 0, `${d.sessionsToday || 0} sesiones`)}
            ${metric('Vistas 7 días', d.visits7d || 0, `${d.sessions7d || 0} sesiones`)}
            ${metric('Vistas 30 días', d.visits30d || 0, `${d.sessions30d || 0} sesiones`)}
          </div>
        </section>

        <section class="panel admin-analytics-secondary">
          <div class="panel-head"><h2>Conversión de acceso</h2><small>Últimos 30 días</small></div>
          <div class="quick-actions">
            ${metric('Conversión', `${Number(d.accessConversion30d || 0).toLocaleString('es-ES')} %`, `${d.accessSubmissionsTracked30d || 0} envíos medidos`)}
          </div>
        </section>

        <section class="panel admin-analytics-secondary"><div class="panel-head"><h2>Vistas por día · últimos 14 días</h2></div>${dailyRows(d.daily)}</section>
        <section class="admin-analytics-grid admin-analytics-secondary">
          <section class="panel"><h2>Páginas · vistas</h2>${listRows(d.topPages,'page','views')}</section>
          <section class="panel"><h2>Dispositivos · vistas</h2>${listRows(d.devices,'device','views')}</section>
          <section class="panel"><h2>Origen · vistas</h2>${listRows(d.referrers,'source','views')}</section>
          <section class="panel"><h2>Países · vistas</h2>${listRows(d.countries,'country','views')}</section>
        </section>
        <section class="panel"><button onclick="adminAnalytics()">↻ Actualizar</button></section>
      `, 'admin');
    } catch (e) {
      ANX.render(`<section class="panel"><button onclick="adminPanel()">← Admin</button>${ANX.msg ? ANX.msg(e.message || 'No se pudo cargar la analítica.','error') : `<p>${esc(e.message)}</p>`}</section>`, 'admin');
    }
  };
  ...
  window.adminAiCustomers = async function () {
    ...
    try {
      const { data, error } = await ANX.supabase.rpc('admin_ai_customer_usage');
      if (error) throw error;
      customerRows = data || [];
      const actions = customerRows.reduce((a,r)=>a+Number(r.actions_month||0),0);
      const tokens = customerRows.reduce((a,r)=>a+Number(r.tokens_month||0),0);
      const provider = customerRows.reduce((a,r)=>a+Number(r.provider_cost_usd||0),0);
      const blocked = customerRows.filter(r=>r.quota_blocked).length;
      ...
      ANX.render(`<section class="panel"><button onclick="adminPanel()">← Admin</button>${ANX.msg(e.message || 'No se pudo cargar el control de clientes.','error')}</section>`, 'admin');
    }
  };

  window.adminAiCustomerEdit = function (userId) {
    analyticsStyles();
    const r = customerRows.find(x => String(x.user_id) === String(userId));
    if (!r) return adminAiCustomers();
  ...
  window.adminAiCustomerSave = async function (userId) {
    const box = document.getElementById('aiBillingStatus');
    if (box) box.innerHTML = ANX.msg('Guardando...');
    try {
      const args = {
        p_user_id:userId,
        p_commercial_plan:value('aiCommercialPlan') || 'sin_configurar',
        p_monthly_action_limit:nullableNumber('aiActionLimit'),
        p_monthly_token_limit:nullableNumber('aiTokenLimit'),
        p_monthly_provider_cost_limit_usd:nullableNumber('aiCostLimit'),
        p_hard_limit:!!document.getElementById('aiHardLimit')?.checked,
        p_warning_percent:nullableNumber('aiWarningPercent') ?? 80,
        p_monthly_fee:nullableNumber('aiMonthlyFee') ?? 0,
        p_price_per_action:nullableNumber('aiPriceAction') ?? 0,
        p_price_per_1k_tokens:nullableNumber('aiPriceTokens') ?? 0,
        p_billing_currency:value('aiCurrency') || 'EUR',
        p_active:!!document.getElementById('aiProfileActive')?.checked,
        p_notes:value('aiBillingNotes') || null
      };
      const { error } = await ANX.supabase.rpc('admin_set_ai_billing_profile', args);
      if (error) throw error;
      if (box) box.innerHTML = ANX.msg('Configuración guardada.','success');
      setTimeout(()=>adminAiCustomers(),500);
    } catch (e) {
      if (box) box.innerHTML = ANX.msg(e.message || 'No se pudo guardar.','error');
    }
  };

  function injectButtons() {
    const headings = Array.from(document.querySelectorAll('.panel-head h2'));
    const target = headings.find(node => /Administración|Control general/i.test(node.textContent || ''));
    const actions = target?.closest('.panel')?.querySelector('.quick-actions');
    if (!actions) return;
    if (!document.getElementById('adminAnalyticsButton')) {
      const b = document.createElement('button');
      b.id = 'adminAnalyticsButton';
      b.setAttribute('onclick','adminAnalytics()');
      b.innerHTML = '<span>▥</span>Analítica';
      actions.appendChild(b);
    }
    if (!document.getElementById('adminAiCustomersButton')) {
      const b = document.createElement('button');
      b.id = 'adminAiCustomersButton';
      b.setAttribute('onclick','adminAiCustomers()');
      b.innerHTML = '<span>€</span>Clientes IA';
      actions.appendChild(b);
    }
  }
