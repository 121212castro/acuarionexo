/* AcuarioNexo · corrección vista ficha */
(function () {
  ...
  function auditHtml(audit, limit) {
    ...
    return `${ANX.msg(`Ficha bloqueada: quedan ${errors.length} campos obligatorios o reglas sin cumplir.`, 'error')}<ul class="small">${errors.slice(0, limit || 6).map(error => `<li>${esc(error)}</li>`).join('')}</ul>${errors.length > (limit || 6) ? `<p class="small">Y ${errors.length - (limit || 6)} incidencias más.</p>` : ''}`;
  }
