...
(function(){
  ...
  function assignData(data,key,text){const value=String(text||'').trim();if(!value)return;if(['temperature','ph','gh','kh','salinity','nitrate','phosphate','calcium','magnesium'].includes(key)){const nums=numbers(value);if(key==='nitrate'||key==='phosphate'){data[`${key}_max`]=nums[nums.length-1]||value;return}if(nums[0])data[`${key}_min`]=nums[0];if(nums[1])data[`${key}_max`]=nums[1];else if(nums[0])data[`${key}_max`]=nums[0];return}if(/(_cm|_liters|_years|_days|_watts|grams_per_liter)$/.test(key)){const n=numbers(value)[0];data[key]=n?Number(n):value;return}data[key]=value;}
  ...
  function parseStructuredFicha(parsed, fallbackType){
    const type=String(parsed.entry_type||fallbackType||'').trim();
    if(!type||!S().CONTRACTS[type])throw new Error(`Tipo de ficha no permitido: ${type||'vacío'}.`);
  ...
  function assertChatFichaComplete(entry){const audit=S().audit(entry);if(audit.approved)return audit;const details=auditText(audit);throw new Error(`La ficha del Chat no cumple el contrato de AcuarioNexo y no se guardó.${details?`\n${details}`:''}`)}
  window.mostrarCrearFichaDesdeChat=function(){
    if(!isAdmin())return;
    const app=byId('app');if(!app)return;
    const box=document.createElement('section');box.className='panel library-detail';box.id='chatCreatePanel';box.innerHTML=`<button onclick="document.getElementById('chatCreatePanel')?.remove()">Cerrar</button><h2>Crear ficha desde texto del Chat</h2><div class="notice">Selecciona el tipo, pega la ficha completa con el bloque JSON de AcuarioNexo y pulsa Crear ficha. La app auditará todo antes de guardar y rechazará cualquier campo inválido.</div><label>Tipo de ficha</label><select id="chatCreateType">${typeOptions()}</select><label>Ficha completa creada por ChatGPT</label><textarea id="chatCreateText" placeholder="Pega aquí toda la ficha"></textarea><button class="primary" onclick="crearFichaDesdeChat()">Crear ficha</button><div id="chatCreateStatus"></div>`;app.prepend(box);box.scrollIntoView({behavior:'smooth',block:'start'})}
  window.crearFichaDesdeChat=async function(){const box=byId('chatCreateStatus');try{const A=anx(), supabase=A.supabase, state=A.state;if(!isAdmin())throw new Error('Esta acción está restringida al panel Admin.');if(!state?.user?.id)throw new Error('Sesión no disponible.');const selected=val('chatCreateType')||'pez_marino', text=val('chatCreateText');if(!text.trim())throw new Error('Pega primero la ficha completa.');const type=detectType(text,selected);const parsed=parseFicha(text,type);const finalType=parsed.entry_type||type;if(!parsed.title)throw new Error('No encontré el nombre común/producto. Usa "Producto:" o "Nombre:".');if(!parsed.sources.length)throw new Error('No encontré el apartado Fuentes. Añade al menos nombres de fuentes o URLs.');if(!parsed.sources.some(s=>s.url)){parsed.data.sources_pending_url=true;parsed.sections.sources_note='Fuentes pegadas sin URL. Revisar y añadir enlaces antes de publicar.'}const row={user_id:state.user.id,title:parsed.title,scientific_name:parsed.scientific_name||null,entry_type:finalType,status:'review',visibility:'public',summary:parsed.summary||null,sections:parsed.sections,data:parsed.data,tags:parsed.tags,identity_confirmed:true,confidence:null,identify_result:{source:'chat_paste',identity_confirmed:true,title:parsed.title,scientific_name:parsed.scientific_name,entry_type:finalType,selected_type:selected,auto_detected_type:type,json_entry_type:parsed.entry_type||null},sources:parsed.sources,ai_model:'chat-paste-manual',ai_generated_at:new Date().toISOString(),updated_at:new Date().toISOString()};assertChatFichaComplete(row);if(box)box.innerHTML=msg(`Contrato validado. Creando ficha como ${finalType}...`,'notice');const{data,error}=await supabase.from('library_entries').insert(row).select('*').single();if(error)throw error;if(window.biblioteca)await window.biblioteca();if(window.formFicha)setTimeout(()=>window.formFicha(data.id),300);if(box)box.innerHTML=msg(`Ficha creada como ${finalType} y validada.`, 'success')}catch(e){if(box)box.innerHTML=msg(e.message,'error')}};
  function isLibraryScreen(){return document.querySelector('.bottom-nav button.active small')?.textContent.trim()==='Biblioteca'}
  function injectButton(){const old=byId('createFromChatBtn');if(!isLibraryScreen()||!isAdmin()){old?.remove();return}const panel=document.querySelector('.panel .panel-head');if(!panel||old)return;const btn=document.createElement('button');btn.id='createFromChatBtn';btn.className='primary';btn.textContent='Crear ficha desde Chat';btn.onclick=window.mostrarCrearFichaDesdeChat;panel.appendChild(btn)}
