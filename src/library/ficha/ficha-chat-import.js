...
(function(){
  ...
  function detectType(text, selected){
    const t=norm(text);
    if(/clasificar\s+como\s+test|\btest\b|checker|fotometro|fotómetro|colorimetro|colorímetro|reactivo|reagent|medidor\s+de\s+ph|\bph\b/.test(t))return'test';
    if(/clasificar\s+como\s+medicamento|medicamento|tratamiento|antibiotico|antibiótico|antiparasitario|medicacion|medicación/.test(t))return'medicamento';
    if(/clasificar\s+como\s+sal|\bsal\b|sal marina|salt mix/.test(t))return'sal';
    if(/clasificar\s+como\s+aditivo|aditivo|buffer|suplemento|calcio|magnesio|kh|alcalinidad/.test(t))return'aditivo';
  ...
  function parseSourcesRaw(text){
    ...
    return rows.map((line,i)=>{
      ...
      let name=parts[0]&&!/^https?:\/\//i.test(parts[0])?parts[0]:'';
      if(!name&&url){try{name=new URL(url).hostname}catch(_){name=`Fuente ${i+1}`}}
      return{url,name:name||`Fuente ${i+1}`,used_for:parts[2]||'Ficha creada desde texto del Chat',pending_url:!url};
    });
  }
  ...
  function numbers(text){return String(text||'').replace(/,/g,'.').match(/-?\d+(?:\.\d+)?/g)||[]}
  function assignData(data,key,text){const value=String(text||'').trim();if(!value)return;
    if(['temperature','ph','gh','kh','salinity','nitrate','phosphate','calcium','magnesium'].includes(key)){const nums=numbers(value);if(key==='nitrate'||key==='phosphate'){data[`${key}_max`]=nums[nums.length-1]||value;return}if(nums[0])data[`${key}_min`]=nums[0];if(nums[1])data[`${key}_max`]=nums[1];else if(nums[0])data[`${key}_max`]=nums[0];return}
    if(/(_cm|_liters|_years|_days|_watts|grams_per_liter)$/.test(key)){const n=numbers(value)[0];data[key]=n?Number(n):value;return}
    data[key]=value;
  }
  function extractInline(text, label){const m=String(text||'').match(new RegExp(label+'\\s*:\\s*([^\\n.]+)','i'));return m?m[1].trim():''}
  function parseFicha(text,type){const aliases=aliasMap(type), blocks={}, urls=[];let current='';String(text||'').split(/\n/).forEach(raw=>{const line=raw.trim();const found=line.match(/https?:\/\/\S+/gi);if(found)found.forEach(u=>urls.push(u.replace(/[.,;]+$/,'')));if(!line||line.startsWith('```'))return;const clean=line.replace(/^[-*•]\s*/,'').trim();const parts=clean.split(/[:：]/);let head='',body='';if(parts.length>1){head=norm(parts.shift());body=parts.join(':').trim()}else head=norm(clean);const key=aliases.get(head);if(key){current=key;if(body)blocks[current]=[blocks[current],body].filter(Boolean).join('\n');return}if(current)blocks[current]=[blocks[current],clean].filter(Boolean).join('\n')});
    ...
    return{title,scientific_name:scientific,summary,tags,data,sections,sources:normalized.concat(pending)};
  }
  window.mostrarCrearFichaDesdeChat=function(){const app=byId('app');if(!app)return;const box=document.createElement('section');box.className='panel library-detail';box.id='chatCreatePanel';box.innerHTML=`<button onclick="document.getElementById('chatCreatePanel')?.remove()">Cerrar</button><h2>Crear ficha desde texto del Chat</h2><div class="notice">Selecciona el tipo, pega la ficha completa y pulsa Crear ficha. Si el texto indica test, producto, sal, aditivo, alimento, medicamento o equipo, AcuarioNexo lo reclasifica automáticamente.</div><label>Tipo de ficha</label><select id="chatCreateType">${typeOptions()}</select><label>Ficha completa creada por ChatGPT</label><textarea id="chatCreateText" placeholder="Pega aquí toda la ficha"></textarea><button class="primary" onclick="crearFichaDesdeChat()">Crear ficha</button><div id="chatCreateStatus"></div>`;app.prepend(box);box.scrollIntoView({behavior:'smooth',block:'start'})}
  window.crearFichaDesdeChat=async function(){const box=byId('chatCreateStatus');try{const A=anx(), supabase=A.supabase, state=A.state;if(!state?.user?.id)throw new Error('Sesión no disponible.');const selected=val('chatCreateType')||'pez_marino', text=val('chatCreateText');if(!text.trim())throw new Error('Pega primero la ficha completa.');const type=detectType(text,selected);const parsed=parseFicha(text,type);if(!parsed.title)throw new Error('No encontré el nombre común/producto. Usa "Producto:" o "Nombre:".');if(!parsed.sources.length)throw new Error('No encontré el apartado Fuentes. Añade al menos nombres de fuentes o URLs.');if(!parsed.sources.some(s=>s.url)){parsed.data.sources_pending_url=true;parsed.sections.sources_note='Fuentes pegadas sin URL. Revisar y añadir enlaces antes de publicar.'}const row={user_id:state.user.id,title:parsed.title,scientific_name:parsed.scientific_name||null,entry_type:type,status:'review',visibility:'public',summary:parsed.summary||null,sections:parsed.sections,data:parsed.data,tags:parsed.tags,identity_confirmed:true,confidence:null,identify_result:{source:'chat_paste',identity_confirmed:true,title:parsed.title,scientific_name:parsed.scientific_name,entry_type:type,selected_type:selected,auto_detected_type:type},sources:parsed.sources,ai_model:'chat-paste-manual',ai_generated_at:new Date().toISOString(),updated_at:new Date().toISOString()};if(box)box.innerHTML=msg(`Creando ficha como ${type}...`,'notice');const{data,error}=await supabase.from('library_entries').insert(row).select('*').single();if(error)throw error;if(window.biblioteca)await window.biblioteca();if(window.formFicha)setTimeout(()=>window.formFicha(data.id),300);if(box)box.innerHTML=msg('Ficha creada. Revisa fuentes y audita.','success')}catch(e){if(box)box.innerHTML=msg(e.message,'error')}};
  function isLibraryScreen(){return document.querySelector('.bottom-nav button.active small')?.textContent.trim()==='Biblioteca'}
  function injectButton(){const old=byId('createFromChatBtn');if(!isLibraryScreen()){old?.remove();return}const panel=document.querySelector('.panel .panel-head');if(!panel||old)return;const btn=document.createElement('button');btn.id='createFromChatBtn';btn.className='primary';btn.textContent='Crear ficha desde Chat';btn.onclick=window.mostrarCrearFichaDesdeChat;panel.appendChild(btn)}
  const mo=new MutationObserver(()=>injectButton());mo.observe(document.body,{childList:true,subtree:true});setTimeout(injectButton,500);
})();
