...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const identifiedEntry = { ...parsed, entry_type: entryType, data: parsed.data || { culture_type: parsed.culture_type, identification: parsed.sections?.identity } };
    const sources = normalizeSources(parsed.sources); const biologicalOk = !biologicalTypes.has(entryType) || concreteScientificName(parsed.scientific_name) || multiTaxonMicrofauna(identifiedEntry); const productOk = !productTypes.has(entryType) || Boolean(clean(parsed.title) && (brand || clean(parsed.manufacturer))); const confirmed = parsed.identity_confirmed === true && sources.length >= 2 && biologicalOk && productOk;
