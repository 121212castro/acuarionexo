...
as $function$
declare
  is_biological boolean := new.entry_type = any(array[
    'pez_marino','pez_dulce','coral','invertebrado','planta','microfauna','fitoplancton'
  ]);
...
begin
  ...
  is_multitaxon_microfauna :=
    ...
    and coalesce(new.data->>'culture_type', '') || ' ' || coalesce(new.data->>'identification', '')
      ~* '\m(mezcla|multiespecífica|multiespecifica|multiespecífico|multiespecifico)\M'
    and not exists (
      select 1
      from unnest(scientific_taxa) as taxon
      where btrim(taxon) !~ '^[A-Z][a-z-]+( [a-z][a-z-]+( var\. [a-z-]+)?| spp?\.)?$'
    );
  ...
  if tg_op = 'UPDATE' and old.status in ('validated','published') and (
    ...
    new.validation_result := jsonb_build_object(
      'approved', false,
      'errors', jsonb_build_array('La ficha cambió después de validarse y requiere una nueva auditoría.'),
      'audited_at', now()
    );
    approved := false;
  end if;
  ...
  if is_biological and new.status in ('draft','review','validated','published')
    ...
    and (
      coalesce(new.scientific_name, '') !~ '^[A-Z][a-z-]+ [a-z][a-z-]+(\s+var\.\s+[a-z-]+)?$'
      or new.scientific_name ~* '\m(spp?|cf|aff)\.?\M'
    ) then
    raise exception 'La ficha biológica requiere una especie concreta.';
  end if;
  if new.status in ('validated','published') then
    if public.library_real_url_count(new.sources) < 3 then
      raise exception 'Se requieren al menos tres fuentes con URL real.';
    end if;
  ...
  if new.status = 'published' then
    if tg_op = 'INSERT' or old.status <> 'validated' then
      raise exception 'Solo se puede publicar una ficha previamente validada.';
    end if;
