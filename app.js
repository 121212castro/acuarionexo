...
const bibliotecaSelects = {
  biblioteca_fichas: 'id,nombre,nombre_comun,nombre_cientifico,categoria,category,tipo,tipo_ficha,creator_category,grupo,seccion,foto_url,foto,imagen,image_url,url_foto,resumen_rapido,resumen,description,descripcion,descripcion_detallada,notes',
  library_entries: 'id,title,common_name,scientific_name,scientific,category,creator_category,tipo,tipo_ficha,grupo,seccion,photo_url,foto_url,foto,imagen,image_url,url_foto,resumen_rapido,resumen,description,descripcion,notes'
};
...
function bibliotecaTimeout(p, ms = 8000) { return Promise.race([p, new Promise((_, reject) => setTimeout(() => reject(new Error('Tiempo de carga agotado')), ms))]); }
async function bibliotecaTabla(tabla, texto) { try { let q = s.from(tabla).select(bibliotecaSelects[tabla] || '*').limit(texto ? 80 : 36); if (texto) q = tabla === 'biblioteca_fichas' ? q.or('nombre.ilike.%' + texto + '%,nombre_cientifico.ilike.%' + texto + '%,descripcion.ilike.%' + texto + '%') : q.or('title.ilike.%' + texto + '%,scientific_name.ilike.%' + texto + '%,description.ilike.%' + texto + '%'); let r = await bibliotecaTimeout(q); if (r.error) { let fb = s.from(tabla).select('*').limit(texto ? 40 : 24); if (texto) fb = tabla === 'biblioteca_fichas' ? fb.or('nombre.ilike.%' + texto + '%,nombre_cientifico.ilike.%' + texto + '%,descripcion.ilike.%' + texto + '%') : fb.or('title.ilike.%' + texto + '%,scientific_name.ilike.%' + texto + '%,description.ilike.%' + texto + '%'); r = await bibliotecaTimeout(fb); } return r.error || !Array.isArray(r.data) ? [] : r.data.map(x => bibliotecaNorm(x, tabla)).filter(bibliotecaCompleta); } catch (e) { return []; } }
async function bibliotecaDatos(texto) { const res = await Promise.allSettled([bibliotecaTabla('biblioteca_fichas', texto), bibliotecaTabla('library_entries', texto)]); const rows = res.flatMap(x => x.status === 'fulfilled' ? x.value : []); const map = new Map(); rows.forEach(f => { const key = (f.nombre + '|' + f.cientifico).toLowerCase(); if (!map.has(key)) map.set(key, f); }); return Array.from(map.values()); }
