...
function bibliotecaCard(f, i) { return `<article class="library-card" onclick="verFichaBiblioteca(${i})">${f.foto ? `<img src="${esc(f.foto)}" alt="${esc(f.nombre)}" loading="lazy">` : '<div class="library-no-photo">🐠</div>'}<div class="library-card-body"><small>${esc(bibliotecaModuloLabel(bibliotecaModulo(f)))}</small><h3>${esc(f.nombre)}</h3>${f.cientifico ? `<p class="scientific">${esc(f.cientifico)}</p>` : ''}${f.descripcion ? `<p>${esc(f.descripcion).slice(0, 180)}${String(f.descripcion).length > 180 ? '…' : ''}</p>` : ''}${window.q ? `<button onclick='event.stopPropagation();importarAnimalBiblioteca(${JSON.stringify(f.raw).replace(/'/g, '&#039;')})'>Añadir a ${esc(window.q.name || 'mi acuario')}</button>` : ''}</div></article>`; }
