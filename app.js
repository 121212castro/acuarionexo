...
function bibliotecaValor(v) { if (v == null || v === '') return ''; if (Array.isArray(v)) return v.filter(Boolean).join('\\n'); if (typeof v === 'object') return Object.values(v).filter(Boolean).join('\\n'); return String(v); }
function bibliotecaCampo(f, keys) { const raw = f.raw || {}; const pools = [raw, raw.apartados, raw.bloques, raw.modulos, raw.modules, raw.sections, raw.secciones, raw.data, raw.ficha].filter(Boolean); for (const p of pools) { for (const k of keys) { const v = p[k] ?? p[k.replaceAll('_', '-')] ?? p[k.replaceAll('_', ' ')]; const out = bibliotecaValor(v).trim(); if (out) return out; } } return ''; }
