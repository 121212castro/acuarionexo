...
const state = { user:null, aquarium:null, aquariums:[], section:'resumen' };
...
function shell(body,active='inicio'){ render(body + bottomNav(active)); }
...
function animalCard(a){ return `<div class="item">${a.photo_url?`<img src="${esc(a.photo_url)}" style="width:100%;max-height:170px;object-fit:cover;border-radius:14px;margin-bottom:8px">`:''}<b>${esc(a.common_name)}</b><p>${esc(a.scientific_name||'')}</p><p class="small">${esc(catEs(a.category))} · ${esc(a.status||'active')} · Cantidad ${esc(a.quantity||1)}</p>${a.notes?`<p>${esc(a.notes)}</p>`:''}</div>`; }
...
window.animalMenu=function(){ setAqSection('animales'); shell(am('animales')+`<section class="panel"><button onclick="anis()">← Volver</button><h2>Añadir animal</h2><div class="quick-actions"><button onclick="formAnimalManual()"><span>✍️</span>Manual</button></div></section>`,'acuarios'); };
window.formAnimalManual=function(){ setAqSection('animales'); shell(am('animales')+`<section class="panel"><button onclick="animalMenu()">← Volver</button><h2>Animal manual</h2><label>Nombre común</label><input id="anName"><label>Nombre científico</label><input id="anSci"><label>Tipo</label><select id="anCat"><option value="fish">Pez</option><option value="coral">Coral</option><option value="invertebrate">Invertebrado</option><option value="other">Otro</option></select><label>Cantidad</label><input id="anQty" type="number" min="1" value="1"><label>Estado</label><select id="anStatus"><option value="active">Activo</option><option value="quarantine">Cuarentena</option><option value="hospital">Hospital</option><option value="archived">Archivado</option></select><label>Notas</label><textarea id="anNotes"></textarea><button class="primary" onclick="saveAnimal()">Guardar animal</button><div id="x"></div></section>`,'acuarios'); };
window.saveAnimal=async function(){ try{ const name=val('anName'); if(!name) throw new Error('Pon el nombre del animal.'); const row={user_id:state.user.id,aquarium_id:window.q.id,common_name:name,scientific_name:val('anSci')||null,category:val('anCat')||'other',quantity:Number(val('anQty')||1),status:val('anStatus')||'active',notes:val('anNotes')||null}; const {error}=await s.from('animals').insert(row); if(error) throw error; anis(); }catch(e){ $('x').innerHTML=msg(e.message,'error'); } };
