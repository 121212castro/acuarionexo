...
async function bibliotecaDatos(texto) { const res = await bibliotecaTabla(texto); window.__bibliotecaWarnings = res.warning ? [res.warning] : []; const map = new Map(); res.rows.forEach(f => { const key = (f.nombre + '|' + f.cientifico).toLowerCase(); if (!map.has(key)) map.set(key, f); }); return Array.from(map.values()); }
