...
const bibliotecaModulos = [
  { label: 'Peces marinos', desc: 'Fichas de peces marinos, comportamiento, alimentacion y compatibilidad.', icon: '🐠', grupo: 'Peces' },
  { label: 'Peces de agua dulce', desc: 'Fichas de dulce por especie y variedad.', icon: '🐟', grupo: 'Peces' },
  { label: 'Corales', desc: 'SPS, LPS, blandos, ubicacion, luz, flujo y cuidados.', icon: '🪸', grupo: 'Corales' },
  { label: 'Invertebrados', desc: 'Gambas, caracoles, cangrejos, estrellas y otros invertebrados.', icon: '🦐', grupo: 'Invertebrados' },
  { label: 'Plantas y algas', desc: 'Plantas de dulce, macroalgas y algas utiles o problematicas.', icon: '🌿', grupo: 'Plantas y algas' },
  { label: 'Microfauna', desc: 'Copepodos, rotiferos, artemia, fitoplancton e infusorios.', icon: '∞', grupo: 'Microfauna' },
  { label: 'Medicamentos', desc: 'Tratamientos, cuarentena, dosis y observaciones.', icon: '💊', grupo: 'Productos y equipo' },
  { label: 'Productos y sales', desc: 'Sales, aditivos, tests, alimentos y consumibles.', icon: '🧂', grupo: 'Productos y equipo' },
  { label: 'Equipamiento', desc: 'Bombas, luces, skimmer, filtros, calentadores y material tecnico.', icon: '⚙️', grupo: 'Productos y equipo' }
];
function bibliotecaGrupo(c) { const k = String(c || '').toLowerCase(); if (k.includes('coral')) return 'Corales'; if (k.includes('fish') || k.includes('pez') || k.includes('peces') || k.includes('marino') || k.includes('dulce')) return 'Peces'; if (k.includes('invert') || k.includes('crust') || k.includes('molus')) return 'Invertebrados'; if (k.includes('plant') || k.includes('alga')) return 'Plantas y algas'; if (k.includes('micro')) return 'Microfauna'; if (k.includes('product') || k.includes('producto') || k.includes('sal') || k.includes('equip') || k.includes('medic') || k.includes('test') || k.includes('alimento')) return 'Productos y equipo'; return 'General'; }
function bibliotecaNorm(x) { return { nombre: x.title || x.nombre || x.nombre_comun || x.common_name || x.nombre_cientifico || x.scientific_name || 'Ficha sin nombre', cientifico: x.scientific_name || x.nombre_cientifico || x.scientific || '', categoria: x.category || x.tipo || x.tipo_ficha || x.grupo || x.seccion || 'general', foto: x.photo_url || x.foto_url || x.foto || x.imagen || x.image_url || x.url_foto || '', descripcion: x.description || x.descripcion || x.descripcion_detallada || x.resumen || x.notes || '', raw: x }; }
...
function bibliotecaCard(f) { return `<article class="library-card">${f.foto ? `<img src="${esc(f.foto)}" alt="${esc(f.nombre)}" loading="lazy">` : '<div class="library-no-photo">🐠</div>'}<div class="library-card-body"><small>${esc(bibliotecaGrupo(f.categoria))}</small><h3>${esc(f.nombre)}</h3>${f.cientifico ? `<p class="scientific">${esc(f.cientifico)}</p>` : ''}${f.descripcion ? `<p>${esc(f.descripcion).slice(0, 180)}${String(f.descripcion).length > 180 ? '…' : ''}</p>` : ''}${window.q ? `<button onclick='importarAnimalBiblioteca(${JSON.stringify(f.raw).replace(/'/g, '&#039;')})'>Añadir a ${esc(window.q.name || 'mi acuario')}</button>` : ''}</div></article>`; }
function bibliotecaModulosHtml(lista) { return `<div class="library-modules">${bibliotecaModulos.map(m => { const n = lista.filter(f => bibliotecaGrupo(f.categoria) === m.grupo).length; return `<button onclick="filtrarBibliotecaModulo('${esc(m.grupo)}')"><b>${esc(m.icon)} ${n}</b><span>${esc(m.label)}</span><small>${esc(m.desc)}</small></button>`; }).join('')}</div>`; }
window.renderBibliotecaLista = function(lista, modulo) { const cont = $('bibliotecaResultados'); if (!cont) return; const filtrada = modulo ? lista.filter(f => bibliotecaGrupo(f.categoria) === modulo) : lista; window.__bibliotecaListaActual = lista; cont.innerHTML = bibliotecaModulosHtml(lista) + `<div class="library-section-title"><h3>${esc(modulo || 'Fichas disponibles')}</h3><p class="small">${filtrada.length} fichas encontradas.</p></div>` + (filtrada.length ? `<div class="library-grid">${filtrada.map(bibliotecaCard).join('')}</div>` : msg('No encontré fichas con esa búsqueda o módulo.')); };
...
window.inventario = function() { page('Inventario', '<p>Equipamiento, stock de reactivos y aditivos.</p>', 'inicio'); };
