...
function bibliotecaFiltro(tabla, q, texto) { if (!texto) return q; const clean = String(texto).replace(/[%,]/g, ' ').trim(); if (!clean) return q; return tabla === 'biblioteca_fichas' ? q.or('nombre.ilike.%' + clean + '%,nombre_cientifico.ilike.%' + clean + '%,descripcion.ilike.%' + clean + '%') : q.or('title.ilike.%' + clean + '%,scientific_name.ilike.%' + clean + '%,description.ilike.%' + clean + '%'); }
async function bibliotecaTabla(tabla, texto) { try { const q = bibliotecaFiltro(tabla, s.from(tabla).select('*').limit(texto ? 80 : 40), texto); const r = await bibliotecaTimeout(q); if (r.error) return { rows: [], warning: tabla + ': ' + r.error.message }; const rows = Array.isArray(r.data) ? r.data.map(x => bibliotecaNorm(x, tabla)).filter(bibliotecaCompleta) : []; return { rows, warning: '' }; } catch (e) { return { rows: [], warning: tabla + ': ' + e.message }; } }
async function bibliotecaDatos(texto) { const res = await Promise.all([bibliotecaTabla('biblioteca_fichas', texto), bibliotecaTabla('library_entries', texto)]); const warnings = res.map(x => x.warning).filter(Boolean); window.__bibliotecaWarnings = warnings; const rows = res.flatMap(x => x.rows); const map = new Map(); rows.forEach(f => { const key = (f.nombre + '|' + f.cientifico).toLowerCase(); if (!map.has(key)) map.set(key, f); }); return Array.from(map.values()); }
...
window.buscarFichasAcuario = async function() { const texto = val('fichasAcuarioSearch'); const cont = $('fichasAcuarioResultados'); if (cont) cont.innerHTML = msg('Cargando fichas completas desde Supabase...'); window.renderFichasAcuarioLista(await bibliotecaDatos(texto), null); };
